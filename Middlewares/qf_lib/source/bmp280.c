...
#define BMP280_MODE                                                                 \
    ((BMP280_PRESSURE_OSR << 2) | (BMP280_TEMPERATURE_OSR << 5) | BMP280_NORMAL_MODE)
...
void bmp280_attach_trans_cb(bmp280_trans_cb_t* cbs) { _bmp_trans_cb = *cbs; }

void bmp280_sleep(uint8_t en)
{
    if (en)
        _bmp_trans_cb.bmp_write_byte(
            BMP280_ADDR, BMP280_CTRL_MEAS_REG, BMP280_SLEEP_MODE);
}

void bmp280_reset()
{
    _bmp_trans_cb.bmp_write_byte(BMP280_ADDR, BMP280_RST_REG, 0xb6);
}
...
#if BMP280_DEBUG
        BMP280_DEBUG_LOG_PRINTF(
            "bmp280 initialization failed, iic trans cb is not attach\n");
#endif
        return 1;
    }
...
#if BMP280_DEBUG
        BMP280_DEBUG_LOG_PRINTF("bmp280 initialization failed,please chek hardware "
                                "connect and iic slave address\n");
#endif
        return 2;
    }

    _bmp_trans_cb.bmp_read_bytes(BMP280_ADDR,
...
uint8_t bmp280_read(int16_t* temp, int32_t* pressure)
{
    ...
    // Temperature
    var1 = (((float)adc_T) / 16384.0 - ((float)bmp280Cal.dig_T1) / 1024.0)
        * ((float)bmp280Cal.dig_T2);
    var2 = ((((float)adc_T) / 131072.0 - ((float)bmp280Cal.dig_T1) / 8192.0)
            * (((float)adc_T) / 131072.0 - ((float)bmp280Cal.dig_T1) / 8192.0))
        * ((float)bmp280Cal.dig_T3);
    ...
    var1 = (((float)bmp280Cal.dig_P3) * var1 * var1 / 524288.0
            + ((float)bmp280Cal.dig_P2) * var1)
        / 524288.0;
