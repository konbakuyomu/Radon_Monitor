...
void wire_fast_set_delay(uint8_t wait) { fast_delay = wait; }
#endif
...
void wire_set_delay(wire_handle_t handle, uint8_t wait) { handle->delay = wait; }

void wire_set_timeout(wire_handle_t handle, uint16_t tic) { handle->timeout = tic; }
...
uint8_t wire_available(wire_handle_t handle) { return handle->buffer_r_c; }
