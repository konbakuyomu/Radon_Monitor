...
/*
   ...
   method of the editor. 模拟IIC主机库 By启凡科创 适用于任意C/C++平台MCU

    version:v3.0.0  2023-3-8
*/

// #include "LIB_CONFIG.h"
// //包含启凡科创配置文件，如不是在此环境下自行注释并包含必要的变量类型定义等头文件
...
#ifndef WIRE_EN
#define WIRE_EN                                                                     \
    0 // 如果配置文件未使能，则可更改此参数用户自定义，0禁用模块不不编译，1启用模块
#endif
...
/*
    IIC地址类型定义，0为8位地址类型，1为7位地址类型
        根据自己的应用场景修改地址模式  如SSD1306等OLED驱动8位地址为0x78，7位位0x3c
...
/**
 * @brief IIC
 * GPIO初始化函数，请根据自己的平台和接线到.c开头进行添加，对应GPIO应初始化为上拉输出模式，速度自行根据需求定义
 */
...
/*
    高速模式下时钟默认等待间隔 0-255
   等待时间越短，IIC速率越快,但是过快的速度可能导致从机跟不上无法正常使用，根据实际情况调整
 ...
 * @param len 写入字节数
 */
void wire_fast_write_bytes(uint8_t slave_add,
                           uint8_t reg_add,
                           uint8_t* dat,
                           uint16_t len);
...
/**
 * @brief
 * 高速固定IO模式，设置通信时钟延迟，值越大总线速率越慢，请根据设备速率和平台自行测试延时大小多少通信正常
 *
...
typedef struct {
    ...
    uint8_t scl;                    // scl io号
    void (*gpio_set_level)(
        uint8_t io, uint8_t level); // 设置管脚高低电平函数,io:io号，level：电平0/1
    uint8_t (*gpio_get_level)(
        uint8_t io); // 读取管脚高低电平函数，io:io号，返回高低电平0/1
 ...
 * @return 成功返回1，失败返回0
 */
uint8_t wire_read_bytes(wire_handle_t handle,
                        uint8_t slave_add,
                        uint8_t reg_add,
                        uint8_t* dat,
                        uint8_t len);
 ...
 * @return 成功返回1，失败返回0
 */
uint8_t wire_write_byte(wire_handle_t handle,
                        uint8_t slave_add,
                        uint8_t reg_add,
                        uint8_t value);
 ...
 * @return 成功返回1，失败返回0
 */
uint8_t wire_write_bytes(wire_handle_t handle,
                         uint8_t slave_add,
                         uint8_t reg_add,
                         uint8_t* dat,
                         uint16_t len);
